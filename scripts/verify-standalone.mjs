...
try {
  ...
  await writeFile(join(appRoot, "test", "sdk-contract.mjs"), `import { FRAMEKIT_SDK_CONFIG_VERSION, FramekitValidationError, upgradeFramekitClientConfig } from "@framekit/sdk";
const upgraded = upgradeFramekitClientConfig({ version: 1, baseUrl: "http://localhost" });
if (FRAMEKIT_SDK_CONFIG_VERSION !== 2 || upgraded.config.version !== 2 || upgraded.config.retry !== undefined || typeof FramekitValidationError !== "function") {
